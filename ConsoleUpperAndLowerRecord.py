...
from Projects.PC.TestCases.Console.Admin.console_lower_record_course import console_lower_record_course
from Projects.PC.TestCases.Console.Admin.console_upper_record_course import console_upper_record_course
...
@test_scenario()
def ConsoleUpperAndLowerRecord(*args, **kwargs):
    """
    讲师登录-创建录播课-后台登录-录播课审核通过-后台下架录播课-后台上架录播课
    :param args:
        0: ProjectBase
        1: mysql
        2: mongo
    ...
    data = Data.TEST_DATA['ConsoleUpperAndLowerRecord']
    # 用户登录
    ...
    # 后台下架录播课
    clrc = console_lower_record_course(*args, **kwargs)
    # 后台上架录播课
    curc = console_upper_record_course(*args, **kwargs)
