...
from Projects.PC.TestCases.User.Teacher.teacher_create_public_group import teacher_create_public_group
...
@test_scenario()
def TeacherCreatePublicGroup(*args, **kwargs):
    """
    讲师登录-发布直播讲堂-后台登录-直播讲堂审核通过-讲师创建直播讲堂学友群
    :param args:
        0: ProjectBase
        1: mysql
        2: mongo
    ...
    # test_data列表
    data = Data.TEST_DATA['TeacherCreatePublicGroup']
    # 用户登录
    ...
    try:
        ...
        Assert.is_equal('待审核', cppc['review_detail_first_class_pending_state'])
        Assert.is_equal('审核通过', cppc['review_detail_first_class_pass_state'])
        Assert.is_equal('通过', cppc['success_text'])
        # 讲师创建直播讲堂学友群
        tcpg = teacher_create_public_group(*args, **kwargs)
        Assert.is_equal(data['teacher_create_public_class']['In']['product_name'], tcpg['product_name'])
    finally:
        # 重置讲师日历表信息
