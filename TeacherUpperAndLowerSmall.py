...
from Projects.PC.TestCases.User.Teacher.teacher_lower_small_course import teacher_lower_small_course
from Projects.PC.TestCases.User.Teacher.teacher_upper_small_course import teacher_upper_small_course
...
@test_scenario()
def TeacherUpperAndLowerSmall(*args, **kwargs):
    """
    讲师登录-发布直播小班课-后台登录-直播小班课审核通过-讲师下架直播小班课-讲师上架直播小班课
    :param args:
        0: ProjectBase
        1: mysql
        2: mongo
    ...
    # test_data列表
    data = Data.TEST_DATA['TeacherUpperAndLowerSmall']
    # 用户登录
    ...
    try:
        ...
        # 下架直播小班课
        tlsc = teacher_lower_small_course(*args, **kwargs)
        Assert.is_equal(data['teacher_create_small_class']['In']['product_name'], tlsc['on_shelf_product_name'])
        Assert.is_equal(data['teacher_create_small_class']['In']['product_name'], tlsc['off_shelf_product_name'])
        # 上架直播小班课
        tusc = teacher_upper_small_course(*args, **kwargs)
        Assert.is_equal(data['teacher_create_small_class']['In']['product_name'], tusc['off_shelf_product_name'])
        Assert.is_equal(data['teacher_create_small_class']['In']['product_name'], tusc['on_shelf_product_name'])
    finally:
        # 重置讲师日历表信息
