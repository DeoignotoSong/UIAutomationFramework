...
from Projects.PC.TestCases.Organization.Institute.institute_edit_no_pass_public import institute_edit_no_pass_public
...
@test_scenario()
def InstituteEditNoPassPublic(*args, **kwargs):
    """
    机构院校主账号登录-发布直播讲堂-后台登录-直播讲堂审核不通过-机构编辑审核不通过直播讲堂
    :param args:
        0: ProjectBase
        1: mysql
        2: mongo
    ...
    # 初始化文件
    data = Data.TEST_DATA['InstituteEditNoPassPublic']
    # 机构登录
    ...
    try:
        ...
        ienpp = institute_edit_no_pass_public(*args, **kwargs)
        Assert.is_equal(data['institute_create_public_class']['In']['product_name'], ienpp['edit_product_name'])
    finally:
        # 重置讲师日历时间
