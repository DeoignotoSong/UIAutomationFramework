...
from Projects.PC.TestCases.User.Teacher.institute_teacher_public_group import institute_teacher_public_group
...
@test_scenario()
def InstituteTeacherPublicGroup(*args, **kwargs):
    """
    机构院校主账号登录-发布直播讲堂-后台登录-直播讲堂审核通过-机构讲师登录-创建直播讲堂学友群
    :param args:
        0: ProjectBase
        1: mysql
        2: mongo
    ...
    # 初始化文件
    data = Data.TEST_DATA['InstituteTeacherPublicGroup']
    # 机构登录
    ...
    try:
        ...
        Assert.is_equal(icpc['teacher_name'], ul['user_name'])
        # 机构讲师创建直播讲堂学友群
        itpg = institute_teacher_public_group(*args, **kwargs)
        Assert.is_equal(data['institute_create_public_class']['In']['product_name'], itpg['product_name'])
    finally:
        # 重置讲师日历时间
