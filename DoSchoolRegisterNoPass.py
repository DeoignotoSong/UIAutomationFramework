...
@test_scenario()
def DoSchoolRegisterNoPass(*args, **kwargs):
    """
    国内院校注册-提交审核信息-后台登录-院校信息完善-院校审核不通过
    :param args:
        0: ProjectBase
        1: mysql
        2: mongo
    ...
    # 初始化文件
    data = Data.TEST_DATA['DoSchoolRegisterNoPass']
    # 院校注册
    ir = school_register(*args, **kwargs)
    # 院校登录
    il = organization_login(*args, **kwargs)
    # 输入信息提交审核
    ihr = school_home_review(*args, **kwargs)
    # 管理后台用户登录
    cl = console_login(*args, **kwargs)
    # 补全院校机构信息
    cile = console_school_list_edit(*args, **kwargs)
    # 院校审核不通过
    csrn = console_school_review_no(*args, **kwargs)
