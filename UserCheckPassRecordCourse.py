...
from Projects.PC.TestCases.User.Teacher.us_class_manage_record_state import us_class_manage_record_state
...
@test_scenario()
def UserCheckPassRecordCourse(*args, **kwargs):
    """
    登录-创建录播课-录播课审核通过-课程管理查看课程状态
    :param args:
        0: ProjectBase
        1: mysql
        2: mongo
    ...
    data = Data.TEST_DATA['UserCheckPassRecordCourse']
    # 用户登录
    ...
    # 课程管理查看课程状态
    cmrs = us_class_manage_record_state(*args, **kwargs)
    Assert.is_containing(data['user_create_record_course']['In']['product_name'], cmrs['product_name'])
    Assert.is_equal(data['user_create_record_course']['In']['sub_product_name'], cmrs['sub_product_name'])
