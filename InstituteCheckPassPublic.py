...
from Projects.PC.TestCases.Organization.Institute.institute_check_pass_public import institute_check_pass_public
...
@test_scenario()
def InstituteCheckPassPublic(*args, **kwargs):
    """
    机构院校主账号登录-发布直播讲堂-后台登录-直播讲堂审核通过-机构查看审核通过直播讲堂
    :param args:
        0: ProjectBase
        1: mysql
        2: mongo
    ...
    # 初始化文件
    data = Data.TEST_DATA['InstituteCheckPassPublic']
    # 机构登录
    ...
    try:
        ...
        # 机构查看已审核通过的直播讲堂
        icpp = institute_check_pass_public(*args, **kwargs)
        Assert.is_equal(data['institute_create_public_class']['In']['product_name'], icpp['product_name'])
        Assert.is_equal(data['institute_create_public_class']['In']['sub_product_name'], icpp['sub_product_name'])
    finally:
        # 重置讲师日历时间
