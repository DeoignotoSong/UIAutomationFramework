...
@test_scenario()
def InstitutionRvPass(*args, **kwargs):
    """
    国内机构注册-信息完善
    :param args:
        0: ProjectBase
    ...
    # 初始化文件
    data = Data.TEST_DATA['InstitutionRvPass']
    # 机构注册
    ...
    # 审核通过
    cinrp = console_institution_rv_pass(*args, **kwargs)
    # 用户登录
    ol2 = organization_login(*args, **kwargs)
    # 进入机构院校首页
    inh = institution_home(*args, **kwargs)
    Assert.is_containing(data['institution_home_review']['In']['institution_name'], inh['ch_institution_school_name'])
