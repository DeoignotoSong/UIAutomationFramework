...
from Projects.PC.TestCases.Organization.Institute.institute_draft_small_class import institute_draft_small_class
...
@test_scenario()
def InstituteDraftSmallCourse(*args, **kwargs):
    """
    机构院校主账号登录-保存直播小班课草稿
    :param args:
        0: ProjectBase
    ...
    # 初始化文件
    data = Data.TEST_DATA['InstituteDraftSmallCourse']
    # 机构登录
    ol = organization_login(*args, **kwargs)
    # 进入机构院校首页
    inh = institution_home(*args, **kwargs)
    # 发布直播讲堂
    icsc = institute_draft_small_class(*args, **kwargs)
    # 重置讲师日历时间
