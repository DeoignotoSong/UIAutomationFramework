...
from Projects.PC.TestCases.Organization.Institute.institute_lower_public import institute_lower_public
from Projects.PC.TestCases.Organization.Institute.institute_upper_public import institute_upper_public
...
@test_scenario()
def InstituteUpperAndLowerPublic(*args, **kwargs):
    """
    机构院校主账号登录-发布直播讲堂-后台登录-直播讲堂审核通过-机构下架课程-机构上架课程
    :param args:
        0: ProjectBase
        1: mysql
        2: mongo
    ...
    # 初始化文件
    data = Data.TEST_DATA['InstituteUpperAndLowerPublic']
    # 机构登录
    ...
    try:
        ...
        # 机构下架直播讲堂
        ilp = institute_lower_public(*args, **kwargs)
        Assert.is_equal(data['institute_create_public_class']['In']['product_name'], ilp['on_shelf_product_name'])
        Assert.is_equal(data['institute_create_public_class']['In']['product_name'], ilp['off_shelf_product_name'])
        # 机构上架直播讲堂
        iup = institute_upper_public(*args, **kwargs)
        Assert.is_equal(data['institute_create_public_class']['In']['product_name'], iup['off_shelf_product_name'])
        Assert.is_equal(data['institute_create_public_class']['In']['product_name'], iup['on_shelf_product_name'])
    finally:
        # 重置讲师日历时间
