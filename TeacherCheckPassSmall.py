...
from Projects.PC.TestCases.User.Teacher.teacher_check_pass_small import teacher_check_pass_small
...
@test_scenario()
def TeacherCheckPassSmall(*args, **kwargs):
    """
    讲师登录-发布直播小班课-后台登录-直播小班课审核通过-讲师查看已审核通过的直播小班课
    :param args:
        0: ProjectBase
        1: mysql
        2: mongo
    ...
    # test_data列表
    data = Data.TEST_DATA['TeacherCheckPassSmall']
    # 用户登录
    ...
    try:
        ...
        # 讲师查看已审核通过的直播小班课
        tcps = teacher_check_pass_small(*args, **kwargs)
        Assert.is_equal(data['teacher_create_small_class']['In']['product_name'], tcps['product_name'])
        Assert.is_equal(data['teacher_create_small_class']['In']['sub_product_name'], tcps['sub_product_name'])
    finally:
        # 重置讲师日历表信息
