...
from Projects.PC.TestCases.User.Teacher.teacher_check_no_pass_public import teacher_check_no_pass_public
...
@test_scenario()
def TeacherCheckNoPassPublic(*args, **kwargs):
    """
    讲师登录-发布直播讲堂-后台登录-直播讲堂审核通过-讲师查看未审核通过的直播讲堂
    :param args:
        0: ProjectBase
        1: mysql
        2: mongo
    ...
    # test_data列表
    data = Data.TEST_DATA['TeacherCheckNoPassPublic']
    # 用户登录
    ...
    try:
        ...
        # 讲师查看未审核通过的直播讲堂
        tcnpp = teacher_check_no_pass_public(*args, **kwargs)
        Assert.is_equal(data['teacher_create_public_class']['In']['product_name'], tcnpp['product_name'])
        Assert.is_equal(data['teacher_create_public_class']['In']['sub_product_name'], tcnpp['sub_product_name'])
    finally:
        # 重置讲师日历表信息
